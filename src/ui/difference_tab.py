...
class DifferenceTab(QWidget):
    """
    Вкладка для отображения и анализа разницы между текущим изображением и фоном.
    
    Данный компонент предоставляет интерактивный интерфейс для визуализации результата 
    вычитания фонового изображения из текущего кадра. Включает в себя отображение профиля 
    пучка в виде тепловой карты с проекциями по осям X и Y, а также расчет и 
    отображение ключевых параметров пучка: координаты центроида и RMS размеры.
    
    Основные функциональные возможности:
    - Визуализация разницы между текущим изображением и фоном через тепловую карту
    - Отображение проекций интенсивности по осям X и Y
    - Вычисление и отображение центроида и RMS размеров пучка
    - Отображение информации о камере (название, разрешение, размер пикселя)
    - Автоматическое обновление данных через регулярные интервалы времени
    
    Визуализация включает в себя центрированные координаты в физических единицах (мм),
    что позволяет проводить количественный анализ пространственных характеристик пучка.
    
    Attributes:
        main_window: Ссылка на главное окно приложения
        last_data_hash: Хеш последних отображенных данных для оптимизации перерисовки
        update_timer: Таймер для регулярного обновления содержимого вкладки
        plot_canvas: Canvas для отображения графиков matplotlib
        camera_name_label: Метка с названием камеры
        camera_resolution_label: Метка с разрешением камеры
        camera_pixel_size_label: Метка с размером пикселя камеры
        centroid_x_label: Метка с X-координатой центроида
        centroid_y_label: Метка с Y-координатой центроида
        rms_x_label: Метка с RMS размером по X
        rms_y_label: Метка с RMS размером по Y
    """
    ...
    def init_ui(self):
        ...
        beam_info_panel = QGroupBox("Информация о пучке (разница)")
